...
final class DatadogExporterHandler extends TimeLimitedHandler {
  ...
  private static Map<String, String> attributesToMeta(
    ...
    attributes.entrySet().stream()
        ...
        .forEach(entry ->
            result.put(entry.getKey(), attributeValueToString(entry.getValue()))
        );
    return result;
  }
  ...
  private Collection<List<DatadogSpan>> getTraces(Collection<SpanData> spanDataList) {
    ...
    for (SpanData sd : spanDataList) {
      ...
      final String service = meta.getOrDefault("dd_service", this.service);
      final String type = meta.getOrDefault("dd_type", this.type);
